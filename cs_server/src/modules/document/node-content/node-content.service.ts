...
export class NodeContentService {
  ...
  async getNode(node_id: string): Promise<{ node_id: string; txt: string }[]> {
    return await this.nodeContentRepository.getNodeText( node_id );
  }
  async getHtml({ node_id, txt }): Promise<string> {
    const {
      codebox,
      table,
      image,
